...
trait TodoLogger extends Logger {
  ...
  def droppedTable(name: String): Unit = logger.info(s"Created table $name")
  def failedToDropTable(name: String, reason: String): Unit = logger.info(s"Failed to create table: $name because: $reason")
}
